...
- **Lifecycle**: The lock is not acquired greedily on client launch. It is acquired only when a track begins playing or playback state becomes relevant (e.g. `PlayTrack`, `SyncState` with a current track, or a resume-like `StateUpdate`). If the lock cannot be acquired, the client switches its state to `STANDBY` and suspends network sync/playback. The lock is released on playback `STOPPED`, disconnect, or when the user explicitly opts out.
