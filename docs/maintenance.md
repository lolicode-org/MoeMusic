...
Classloaders for discarded or incompatible standalone jars are closed immediately. Platforms handle discovery issues appropriately (e.g. Minecraft client displays an in-game issue screen on startup, dedicated servers log an ASCII warning banner, and terminal platforms fail closed).
Duplicate [MusicSource.id](../api/src/main/kotlin/org/lolicode/moemusic/api/MusicSource.kt#L124) registrations remain fatal so collisions fail fast with ownership details.
...
Track loudness normalization is a separate client-local gain layer. Sources may populate `TrackInfo.loudness` directly from `getTrackInfo(...)` or return it later through `resolve(...).trackPatch`. `loudness.integratedLufs` enables normalization in general, while `loudness.peak` plus `PeakKind` lets the client decide whether conservative boosting is safe. `client.loudness_normalization.mode` controls whether the client is off, attenuation-only, or conservative-boost. Missing or invalid LUFS falls back to an assumed loud modern master (`-7 LUFS`) so unknown tracks are attenuated conservatively instead of playing at full gain. Effective MoeMusic volume plus transient overrides remains the multiplicative base gain, normalization may rise above that base only within the bounded relative-boost policy, and final output must still never exceed digital full scale.
...
- **Compression Policy**: Payloads under 512 bytes are sent raw (`FLAG_RAW`). Payloads $\ge$ 512 bytes are compressed using standard `Deflater(BEST_SPEED)`. If compressed bytes do not yield smaller size than raw, uncompressed transmission (`FLAG_RAW` or `FLAG_CHUNK_RAW`) is retained.
- **Chunk Sizing and Boundaries**: Chunks are split into 30 KB ($30,720\text{ B}$) slices, well within Minecraft's $32,767\text{ B}$ plugin message limit. Maximum S2C raw payload limit is 2 MB ($2,097,152\text{ B}$), which can be sent compressed or uncompressed across up to 70 chunks (total chunk capacity $2,150,400\text{ B}$). Client-side chunk reassembly maintains up to 4 concurrent transfers with a 15-second TTL per transfer and decompresses up to 2 MB. Client-to-server (`C2S`) raw payload is capped at 64 KB ($65,536\text{ B}$) and must fit within a single 30 KB frame (`encodeSingle`); C2S chunking is strictly disallowed on the server to prevent memory exhaustion.
